...
try:
    ...
    logger.warning(
        f"PostgreSQL unreachable at {settings.POSTGRES_SERVER}:{settings.POSTGRES_PORT}. "
        f"Falling back to local SQLite engine for offline operation. Error: {e}"
    )
...
def get_db_status() -> dict:
    ...
    return {
        "engine": "PostgreSQL" if is_postgres else "SQLite (Offline Fallback)",
        "connected": True,
